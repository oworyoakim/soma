...
export default {
    ...
    actions: {
        ...
        async GET_COURSE_DETAILS ({commit}, payload) {
            try {
                if (!payload.courseId) {
                    return Promise.reject("Course ID required!");
                }
                commit('SET_LOADER', true);
                let response = await httpClient.get('/v1/courses/' + payload.courseId);
                commit('SET_ACTIVE_COURSE', response.data);
                commit('SET_LOADER', false);
                return Promise.resolve(response.data);
            } catch (error) {
                commit('SET_LOADER', false);
